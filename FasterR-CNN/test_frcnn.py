...
# Hier ist nur der Testdaten-Pfad von Interesse, alles andere ist Default
parser = OptionParser()
...
# Konfigurationsdatei config.pickle laden
with open(config_output_filename, 'rb') as f_in:
    C = pickle.load(f_in)

# Resnet50 (Basisnetzwerk) importieren
...
# Mindestwahrscheinlichkeit für die Anzeige von Lokalisierungen
bbox_threshold = 0.8
...
for idx, img_name in enumerate(sorted(os.listdir(img_path))):
    ...
    # Ausgabe Verarbeitungszeit des Bildes
    print('Elapsed time = {}'.format(time.time() - st))
    print(all_dets)
    #cv2.imshow('img', img)
    #cv2.waitKey(0)

    # Bild wird abgespeichert in results_imgs
